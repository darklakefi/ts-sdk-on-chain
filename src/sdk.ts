...
export class DarklakeSDK {
    ...
    constructor(rpcEndpoint: string, commitmentLevel: Commitment, isDevnet: boolean, label: string | null, refCode: string | null) {
      ...
      const provider = new AnchorProvider(this.connection, new Wallet(new Keypair()), { commitment: this.commitment } );
    ...
    public async quote(tokenIn: PublicKey, tokenOut: PublicKey, amountIn: BN): Promise<Quote> {
      try {
        ...
        const [poolKey, tokenMintX, tokenMintY] = this.getPoolAddress(tokenIn_, tokenOut_);
    ...
    public async swapTx(
      ...
      try {
        ...
        const addressLookupTableAccount = await getAddressLookupTable(this.connection, this.isDevnet);

        if (isFromSol) {
          const solToWsolInstructions = await getWrapSolToWsolInstructions(tokenOwner, new BN(amountIn));
    ...
    public async finalizeTx(
      ...
      try {
        ...
        const associatedTokenAccount = await getAssociatedTokenAddress(NATIVE_MINT, settler);
        ...
        const computeBudgetIx = web3.ComputeBudgetProgram.setComputeUnitLimit({ units: 500000 });
    ...
    public async addLiquidityTx(
      ...
      try {
        ...
        let [poolKey, tokenMintX, tokenMintY] = this.getPoolAddress(tokenXPostSol, tokenYPostSol);
        ...
        const addLiquidityInstruction = await this.addLiquidityIx(addLiquidityParamsIx);

        let allInstructions: TransactionInstruction[] = [];

        if (isXSol) {
          const solToWsolInstructions = await getWrapSolToWsolInstructions(user, new BN(maxAmountX));
          allInstructions = allInstructions.concat(solToWsolInstructions);

        // Cant be both
        } else if (isYSol) {
          const solToWsolInstructions = await getWrapSolToWsolInstructions(user, new BN(maxAmountY));
          allInstructions = allInstructions.concat(solToWsolInstructions);
        }
    ...
    public async removeLiquidityTx(
      ...
      try {
        ...
        const associatedTokenX = await getAssociatedTokenAddress(tokenMintX, user, false, tokenXOwner);
        const associatedTokenY = await getAssociatedTokenAddress(tokenMintY, user, false, tokenYOwner);

        const createTokenXAtaIx = await createAssociatedTokenAccountIdempotentInstruction(
          user,
          associatedTokenX,
          user,
          tokenMintX,
        ...
        const createTokenYAtaIx = await createAssociatedTokenAccountIdempotentInstruction(
          user,
          associatedTokenY,
          user,
          tokenMintY,
        ...
        const removeLiquidityInstruction = await this.removeLiquidityIx(removeLiquidityParamsIx);
    ...
    public async initializePoolTx(
      ...
      try {
        ...
        let [_, tokenMintX, tokenMintY] = this.getPoolAddress(tokenXPostSol, tokenYPostSol);
        ...
        const initializePoolInstruction = await this.initializePoolIx(initializePoolParamsIx);
        ...
        if (isXSol) {
          const solToWsolInstructions = await getWrapSolToWsolInstructions(user, new BN(amountX));
          allInstructions = allInstructions.concat(solToWsolInstructions);

        // Cant be both
        } else if (isYSol) {
          const solToWsolInstructions = await getWrapSolToWsolInstructions(user, new BN(amountY));
          allInstructions = allInstructions.concat(solToWsolInstructions);
        }
    ...
    public async loadPool(tokenMintX: PublicKey, tokenMintY: PublicKey): Promise<[PublicKey, PublicKey, PublicKey]> {
      const [poolKey, orderedTokenMintX, orderedTokenMintY] = this.getPoolAddress(tokenMintX, tokenMintY);
    ...
    public async finalizeIx(finalizeParamsIx: FinalizeParamsIx): Promise<TransactionInstruction> {
      try {
        ...
        if (isSlash) {
          ...
          const slashAndAccountMetas = await this.darklakeAmm.getSlashAndAccountMetas(slashParams);
          return new TransactionInstruction({
            programId: this.program.programId,
        ...
        const { proofA, proofB, proofC, publicSignals } =
            await generateProof(privateInputs, publicInputs, !isSettle);
        ...
        if (isSettle) {
          ...
          const settleAndAccountMetas = await this.darklakeAmm.getSettleAndAccountMetas(settleParams, proofParams);
          return new TransactionInstruction({
            programId: this.program.programId,
        ...
        const cancelAndAccountMetas = await this.darklakeAmm.getCancelAndAccountMetas(cancelParams, proofParams);
    ...
    public async addLiquidityIx(addLiquidityParamsIx: AddLiquidityParamsIx): Promise<TransactionInstruction> {
      ...
      const addLiquidityAndAccountMetas = await this.darklakeAmm.getAddLiquidityAndAccountMetas(addLiquidityParams);
    ...
    public async removeLiquidityIx(removeLiquidityParamsIx: RemoveLiquidityParamsIx): Promise<TransactionInstruction> {
      ...
      const removeLiquidityAndAccountMetas = await this.darklakeAmm.getRemoveLiquidityAndAccountMetas(removeLiquidityParams);
    ...
    public async initializePoolIx(initializePoolParamsIx: InitializePoolParamsIx): Promise<TransactionInstruction> {
      ...
      const addLiquidityAndAccountMetas = await this.darklakeAmm.getInitializePoolAndAccountMetas(initializePoolParams, this.isDevnet);
     ...
     */
    private async retryGetOrderAccount(orderKey: PublicKey): Promise<AccountInfo<Buffer> | null> {
      const maxRetries = 5;
