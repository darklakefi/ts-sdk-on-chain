...
import {
  getDarklakeAmmAddLiquidityAccounts,
  getDarklakeAmmCancelAccounts,
  getDarklakeAmmInitializePoolAccounts,
  getDarklakeAmmRemoveLiquidityAccounts,
  getDarklakeAmmSettleAccounts,
  getDarklakeAmmSlashAccounts,
  getDarklakeAmmSwapAccounts,
} from './accounts';
...
export class DarklakeAmm {
  ...
  static loadPool(pool: KeyedAccountInfo): DarklakeAmm {
    ...
    amm.pool = amm.mapDataToPool(
      amm.borshCoder.accounts.decode(
        'Pool',
        Buffer.from(pool.accountInfo.data),
      ),
    );
  ...
  public update(accountMap: Map<PublicKey, AccountData>): void {
    ...
    if (!ammConfigData) {
      throw DarklakeError.fromValidationError(
        ...
        createErrorContext('update', {
          ammConfig: this.pool.ammConfig.toString(),
        }),
      );
    }

    this.ammConfig = this.mapDataToAmmConfig(
      this.borshCoder.accounts.decode(
        'AmmConfig',
        Buffer.from(ammConfigData.data),
      ),
    );
    ...
    this.reserveXBalance = this.parseTokenAccountBalance(
      reserveXData.data,
      reserveXData.owner,
    );
    this.tokenXOwner = reserveXData.owner;

    this.reserveYBalance = this.parseTokenAccountBalance(
      reserveYData.data,
      reserveYData.owner,
    );
    ...
    if (!tokenXData) {
      throw DarklakeError.fromValidationError(
        ...
        createErrorContext('update', {
          tokenMintX: this.pool.tokenMintX.toString(),
        }),
      );
    }
    ...
    if (!tokenYData) {
      throw DarklakeError.fromValidationError(
        ...
        createErrorContext('update', {
          tokenMintY: this.pool.tokenMintY.toString(),
        }),
      );
    }
    ...
    const parsedMintX = unpackMint(
      this.pool.tokenMintX,
      dummyXTokenAccount,
      reserveXData.owner,
    );
    const parsedMintY = unpackMint(
      this.pool.tokenMintY,
      dummyYTokenAccount,
      reserveYData.owner,
    );
  ...
  quote(quoteParams: QuoteParams): Quote {
    ...
    if (isSwapXToY) {
      if (this.tokenXTransferFeeConfig) {
        inputTransferFee = calculateEpochFee(
          this.tokenXTransferFeeConfig,
          BigInt(quoteParams.epoch.toString()),
          BigInt(quoteParams.amount.toString()),
        );
      }
    } else {
      if (this.tokenYTransferFeeConfig) {
        inputTransferFee = calculateEpochFee(
          this.tokenYTransferFeeConfig,
          BigInt(quoteParams.epoch.toString()),
          BigInt(quoteParams.amount.toString()),
        );
      }
    ...
    if (isSwapXToY) {
      if (this.tokenXTransferFeeConfig) {
        outputTransferFee = calculateEpochFee(
          this.tokenXTransferFeeConfig,
          BigInt(quoteParams.epoch.toString()),
          BigInt(quoteParams.amount.toString()),
        );
    ...
    const outputAmount = checkedSub(
      quoteOutput.toAmount,
      new BN(outputTransferFee),
    );
   ...
   */
  async getSwapAndAccountMetas(
    swapParams: SwapParams,
  ): Promise<SwapAndAccountMetas> {
    const { tokenTransferAuthority, salt, minOut, label, inAmount } =
      swapParams;

    const isSwapXToY = swapParams.sourceMint.equals(this.pool.tokenMintX);

    const userTokenAccountWsol = DarklakeAmm.getUserTokenAccount(
      tokenTransferAuthority,
      NATIVE_MINT,
      TOKEN_PROGRAM_ID,
    );

    const userTokenAccountX = DarklakeAmm.getUserTokenAccount(
      tokenTransferAuthority,
      this.pool.tokenMintX,
      this.tokenXOwner,
    );
    const userTokenAccountY = DarklakeAmm.getUserTokenAccount(
      tokenTransferAuthority,
      this.pool.tokenMintY,
      this.tokenYOwner,
    );
    ...
    let serializedLabel = this.serializeOptionalArray(
   ...
   */
  getSettleAndAccountMetas(
    settleParams: SettleParams,
    proofParams: GeneratedProof,
  ): SettleAndAccountMetas {
    ...
    const orderTokenAccountWsol = this.getOrderTokenAccountWsol(
      settleParams.orderOwner,
    );

    const callerTokenAccountWsol = DarklakeAmm.getUserTokenAccount(
      settleParams.settleSigner,
      NATIVE_MINT,
      TOKEN_PROGRAM_ID,
    );
    ...
    const serializedRefCode = this.serializeOptionalArray(
      settleParams.refCode || new Uint8Array(0),
    );
    const serializedLabel = this.serializeOptionalArray(
      settleParams.label || new Uint8Array(0),
    );
  ...
  getCancelAndAccountMetas(
    ...
    const serializedLabel = this.serializeOptionalArray(
      cancelParams.label || new Uint8Array(0),
    );
  ...
  getSlashAndAccountMetas(slashParams: SlashParams): SlashAndAccountMetas {
    ...
    const serializedLabel = this.serializeOptionalArray(
      slashParams.label || new Uint8Array(0),
    );

    const data = new Uint8Array([...discriminator, ...serializedLabel]);
  ...
  getAddLiquidityAndAccountMetas(
    ...
    const userTokenAccountLp = DarklakeAmm.getUserTokenAccount(
      addLiquidityParams.user,
      tokenMintLp,
      TOKEN_PROGRAM_ID,
    );
    ...
    const serializedLabel = this.serializeOptionalArray(
      addLiquidityParams.label || new Uint8Array(0),
    );
    const serializedRefCode = this.serializeOptionalArray(
      addLiquidityParams.refCode || new Uint8Array(0),
    );
   ...
   */
  getRemoveLiquidityAndAccountMetas(
    removeLiquidityParams: RemoveLiquidityParams,
  ): RemoveLiquidityAndAccountMetas {
    const tokenMintLp = DarklakeAmm.getTokenMintLp(this.key);
    const userTokenAccountX = DarklakeAmm.getUserTokenAccount(
      removeLiquidityParams.user,
      this.pool.tokenMintX,
      this.tokenXOwner,
    );
    const userTokenAccountY = DarklakeAmm.getUserTokenAccount(
    ...
    const serializedLabel = this.serializeOptionalArray(
      removeLiquidityParams.label || new Uint8Array(0),
    );
   ...
   */
  getInitializePoolAndAccountMetas(
    initializePoolParams: InitializePoolParams,
    isDevnet: boolean,
  ): InitializePoolAndAccountMetas {
    const discriminator = new Uint8Array([95, 180, 10, 172, 84, 174, 232, 40]);

    const poolKey = DarklakeAmm.getPoolAddress(
      initializePoolParams.tokenX,
      initializePoolParams.tokenY,
    );

    const serializedLabel = this.serializeOptionalArray(
      initializePoolParams.label || new Uint8Array(0),
    );

    const tokenMintLp = DarklakeAmm.getTokenMintLp(poolKey);
    const userTokenAccountX = DarklakeAmm.getUserTokenAccount(
      initializePoolParams.user,
      initializePoolParams.tokenX,
      initializePoolParams.tokenXProgram,
    );
    const userTokenAccountY = DarklakeAmm.getUserTokenAccount(
      initializePoolParams.user,
      initializePoolParams.tokenY,
      initializePoolParams.tokenYProgram,
    );
    const userTokenAccountLp = DarklakeAmm.getUserTokenAccount(
      initializePoolParams.user,
      tokenMintLp,
    ...
    const metadataAccountX = DarklakeAmm.getTokenMetadata(
      initializePoolParams.tokenX,
    );
    const metadataAccountY = DarklakeAmm.getTokenMetadata(
      initializePoolParams.tokenY,
    );

    const createPoolFeeVault = isDevnet
      ? DEVNET_CREATE_POOL_FEE_VAULT
      : MAINNET_CREATE_POOL_FEE_VAULT;

    const poolWsolReserve = DarklakeAmm.getPoolWsolReserve(poolKey);
    const poolTokenReserveX = DarklakeAmm.getPoolReserve(
      poolKey,
      initializePoolParams.tokenX,
    );
    const poolTokenReserveY = DarklakeAmm.getPoolReserve(
      poolKey,
      initializePoolParams.tokenY,
    );
  ...
  parseOrderData(orderData: Uint8Array): Order {
    const rawOrder = this.borshCoder.accounts.decode(
      'Order',
    ...
    if (accountOwner.equals(TOKEN_PROGRAM_ID)) {
      ...
      try {
        ...
        throw DarklakeError.fromValidationError(
          ...
          createErrorContext('getTokenBalance', {
